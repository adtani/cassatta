    ...
    function taskmgmtService($rootScope, app) {
        ...
        function loadTasks(){
        	if(deferred == null){
	        	...
	        	if(tasks == null){
		       	  	app.sqlserver.searchEntities("tasksview","findByAssigneeIdAndStatusNot?assigneeId="+$rootScope.session.user.id+"&status=DONE").then(function(response){
		       	  		if(response.success){
